...
interface ListingRow {
  ...
  owner: string | null; latitude: number | null; longitude: number | null; photos: string[] | null; phone: string | null;
...
function rowToListing(r: ListingRow): Listing {
  return {
    ...
    owner: r.owner, latitude: r.latitude, longitude: r.longitude, photos: r.photos, phone: r.phone,
...
export interface TenderView {
  ...
  kind: 'PUBLIC' | 'PRIVATE'; budgetCve: number | null; deadline: string | null; bids: number;
  status: 'open' | 'closed' | 'draft'; ownerId: string | null;
}
interface TenderRow {
...
function demoToTender(t: (typeof TENDERS)[number]): TenderView {
  return {
    ...
    budgetCve: t.budgetCve, deadline: t.deadline, bids: t.bids, status: 'open', ownerId: null,
  };
}
function tenderRowToView(r: TenderRow): TenderView {
  return {
    ...
    kind: r.kind, budgetCve: r.budget_cve, deadline: r.deadline, bids: r.bids_count, status: r.status, ownerId: r.owner,
  };
}
