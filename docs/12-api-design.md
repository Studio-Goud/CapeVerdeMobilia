...
> The API is served by **Next.js Route Handlers** under `/app/api/v1/*` (modular monolith, canon). Contracts are shared as TypeScript + **zod** (`@kavila/validation`). Must stay consistent with the [Project Canon](_canon.md), the [Technical Architecture](./10-technical-architecture.md), and the [Data Model](./11-data-model.md).
...
Outbound (to partners/PSP later) and inbound (PSP, malware-scan callbacks). All signed (HMAC `X-Kavila-Signature`), timestamped, replay-protected. Inbound handlers are idempotent and enqueue rather than process inline. MVP: media-scan + (deferred) PSP.
