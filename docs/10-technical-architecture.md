...
graph TB
    ...
    Kavila{{Kavíla Platform<br/>Next.js 14 modular monolith}}
    ...
    Kavila --> S3
    Kavila --> Maps
    Kavila -. reads public info .-> GovCV
    Kavila -. future auth/e-sign .-> CMDCV
    Kavila -. later .-> PSP
```
...
| **End-to-end TypeScript** | Shared `@kavila/types`, `@kavila/validation` (zod) from DB to UI eliminates a class of integration bugs. |
...
```
apps/web, apps/admin
  └── call → module public API (e.g. @kavila/modules/listings)
                └── module internals: service, repository, dto, events
                      └── @kavila/database (Prisma) — module owns its tables
```
...
| Validation | zod (`@kavila/validation`) | Shared client+server contracts | — |
...
```mermaid
graph TB
    subgraph Platform["@kavila modular monolith"]
...
- **Product analytics** via `@kavila/analytics` (privacy-respecting; consent-aware given CNPD regime).
...
- UI/URL locale via Next.js App Router segments (`/pt`, `/kea`, `/en`, `/nl`, `/fr`); `@kavila/i18n` holds message catalogues for chrome, while **content** translations live in the DB (`Translations`).
...
sequenceDiagram
    ...
    Buyer->>Kavila: Click "Contact via WhatsApp" on listing
    Kavila->>Kavila: Build wa.me deep link with prefilled, tracked context
    Kavila-->>Buyer: Open WhatsApp (deep link) + log Lead
    Buyer->>Seller: Prefilled message (listing ref)
    Note over Kavila: Lead recorded; conversation happens off-platform (MVP)
```
